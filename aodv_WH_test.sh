...
#         #待機時間ごとのディレクトリを作成
#         SIZE_DIR="${new_dir}/node_${SIZE}/${SIZE}_wait${WAIT}_WH150"
#         mkdir -p "$SIZE_DIR"
...
#             #評価結果を出力するファイル名を作成
#             DEF="${SIZE_DIR}/packet_num_${i}.txt"
#             ./ns3 run "${PROGRAM} --size=${SIZE} --wait_time=${WAIT} --WH_size=150 --time=$TIME --result_file="${DEF}" --iteration=$i > log_node${SIZE}_wait${WAIT}.txt 2>&1"
...
#     #         #評価結果を出力するファイル名を作成
#     #         DEF="${SIZE_DIR}/packet_num_${i}.txt"
#     #         ./ns3 run "${PROGRAM} --size=${SIZE} --end_distance=${DISTANCE} --time=$TIME --result_file="${DEF}" --iteration=$i > log_node${SIZE}_end_distance${DISTANCE}.txt 2>&1"
...
for WH_SIZE in "${WH_SIZES[@]}"; do
    #WHリンクの長さごとのディレクトリを作成
    SIZE_DIR="${SIZE_DIR}/${SIZE}_WH${WH_SIZE}"
    mkdir -p "$SIZE_DIR"
