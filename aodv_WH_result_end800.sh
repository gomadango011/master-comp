...
for WAIT in "${WAIT_TIME[@]}"; do
    ...
    for ((i=1; i<=RUN_COUNT; i++)); do
        ...
        ./ns3 run "${PROGRAM} --end_distance=800 --wait_time=${WAIT} --time=$TIME --result_file="${DEF}" --iteration=$i > log_end${DISTANCE}_wait${WAIT}.txt 2>&1"
