...
RoutingProtocol::Start()
{
    ...
    //     m_whRecvSocket->Bind(InetSocketAddress(Ipv4Address::GetAny(), AODV_PORT));

    //     NS_LOG_DEBUG("WH転送処理を行うコールバックを設定");
    ...
    // if (m_isWhNode)
    // {
    //     NS_LOG_DEBUG("[WH初期化] Start() 内で P2P ソケット初期化を実行");
    //     InitializeWhSockets(2);  // P2P が interface 1 である前提
    // }
}

//WHソケットの初期化
void
RoutingProtocol::InitializeWhTunnel()
{
    if (!m_isWhNode)
        return;

    NS_LOG_INFO("[WH] Initializing tunnel socket");

    m_whPort = 50000; // WHトンネルポート

    m_whSocket = Socket::CreateSocket(GetObject<Node>(), UdpSocketFactory::GetTypeId());
    m_whSocket->Bind(InetSocketAddress(Ipv4Address::GetAny(), m_whPort));

    m_whSocket->SetRecvCallback(
        MakeCallback(&RoutingProtocol::WhTunnelRecv, this));

    NS_LOG_INFO("[WH] Tunnel socket ready on port " << m_whPort);
}

void
RoutingProtocol::SetIsWhNode(bool flag)
{
    m_isWhNode = flag;
                            ...
                            const ErrorCallback& ecb)
{
    ...
    // //WHタグの確認
    // WhForwardTag tag;
    // bool isWhForwarded = false;
    // Ptr<Packet> pwh = p->Copy();

    // if (pwh->PeekPacketTag(tag) && tag.Get())
    // {
    //     isWhForwarded = true;
    //     NS_LOG_DEBUG("[WH] RouteInput: WHタグ付きパケットを受信。ブロードキャスト準備");
    // }

    // //------------------------------------------------------------------
    // // WH経由パケット：AODV制御 / Ping をそのまま再ブロードキャスト
    // //------------------------------------------------------------------
    // if (m_isWhNode && isWhForwarded)
    // {
    //     uint8_t proto = header.GetProtocol();

    //     // ---- AODV制御パケットの場合（UDP + AODV_PORT）----
    //     if (proto == UdpL4Protocol::PROT_NUMBER)
    //     {
    //         Ptr<Packet> tmp = p->Copy();
    //         UdpHeader udp;
    //         if (tmp->PeekHeader(udp))
    //         {
    //             if (udp.GetDestinationPort() == AODV_PORT)
    //             {
    //                 NS_LOG_DEBUG("[WH][RouteInput] AODV制御パケットを再ブロードキャストします");

    //                 // 受信インタフェースに対応する IPv4 アドレスからソケットを取得
    //                 Ipv4InterfaceAddress iface = m_ipv4->GetAddress(iif, 0);
    //                 Ptr<Socket> aodvSocket = FindSocketWithInterfaceAddress(iface);

    //                 if (!aodvSocket)
    //                 {
    //                     NS_LOG_ERROR("[WH][RouteInput] AODV送信用ソケットが見つかりません (iface="
    //                                  << iface.GetLocal() << ")");
    //                 }
    //                 else
    //                 {
    //                     bool oldBlock = m_sendBlocked;
    //                     m_sendBlocked = false; // WH転送なので強制送信

    //                     // そのまま AODV パケットとしてブロードキャスト
    //                     SendTo(aodvSocket,
    //                            p->Copy(),
    ...
    //                     NS_LOG_DEBUG("[WH][RouteInput] AODV制御パケットを 255.255.255.255 へ再送信完了");
    ...
    //     //==============================================================
    //     // ② Ping (ICMPv4) の再ブロードキャスト
    //     //==============================================================
    //     // if (proto == Icmpv4L4Protocol::PROT_NUMBER)
    //     // {
    //     //     NS_LOG_DEBUG("[WH][RouteInput] Ping(ICMPv4) パケットを再ブロードキャストします");

    //     //     Ipv4InterfaceAddress iface = m_ipv4->GetAddress(iif, 0);
    //     //     Ipv4Address src   = iface.GetLocal();
    //     //     Ipv4Address bcast = Ipv4Address("255.255.255.255");

    //     //     Ptr<Packet> icmpPkt = p->Copy();

    //     //     // ★ 古い IPv4 ヘッダが残っていても RemoveHeader して安全に無視できる
    //     //     Ipv4Header oldIp;
    //     //     icmpPkt->RemoveHeader(oldIp);

    //     //     // ★ 新しい IPv4 ヘッダを構築
    //     //     Ipv4Header newIp;
    //     //     newIp.SetSource(src);
    //     //     newIp.SetDestination(bcast);
    //     //     newIp.SetProtocol(Icmpv4L4Protocol::PROT_NUMBER);
    //     //     newIp.SetTtl(64);

    //     //     icmpPkt->AddHeader(newIp);
    ...
    //     //     m_ipv4->Send(icmpPkt,
    //     //                 newIp.GetSource(),
    //     //                 newIp.GetDestination(),
    //     //                 Icmpv4L4Protocol::PROT_NUMBER,
    //     //                 route);

    //     //     m_sendBlocked = oldBlock;

    //     //     NS_LOG_DEBUG("[WH][RouteInput] Ping を 255.255.255.255 へ再送信完了");
    //     //     return true;
    //     // }
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    // // WH ノードであれば、P2P インターフェースなら WH ソケットを初期化する
    // if (m_isWhNode)
    // {
    //     NS_LOG_DEBUG("[NotifyInterfaceUp] interface=" << i 
    //                     << " addr=" << iface.GetLocal());

    //     // このインターフェースが P2P（10.1.2.x）かどうか判断
    //     if (iface.GetLocal().CombineMask("255.255.255.0") == Ipv4Address("10.1.2.0"))
    //     {
    //         NS_LOG_DEBUG("[NotifyInterfaceUp] P2P IF detected → WH 初期化します");
    //         InitializeWhSockets(i);
    //     }
    //     else
    //     {
    //         NS_LOG_DEBUG("[NotifyInterfaceUp] IF=" << i 
    //                         << " は P2P ではないため WH 初期化しません");
    //     }
    // }
...
RoutingProtocol::RecvAodv(Ptr<Socket> socket)
{
    ...
    {
    case AODVTYPE_RREQ: {
        NS_LOG_DEBUG("AODVTYPE_RREQを受信しました");
        RecvRequest(packet, receiver, sender, m_isWhForwardedPacket);
        break;
    }
    case AODVTYPE_RREP: {
        NS_LOG_DEBUG("AODVTYPE_RREPを受信しました");
        RecvReply(packet, receiver, sender, m_isWhForwardedPacket);
        break;
    }
    case AODVTYPE_RERR: {
        NS_LOG_DEBUG("AODVTYPE_RERRを受信しました");
        RecvError(packet, sender, m_isWhForwardedPacket);
        break;
    }
    ...
    case AODVTYPE_VSR: {
        NS_LOG_DEBUG("AODVTYPE_VSRを受信しました");
        RecvVerificationStart(packet, receiver, sender, m_isWhForwardedPacket);
        break;
    }
    case AODVTYPE_AUTH: {
        NS_LOG_DEBUG("AODVTYPE_AUTHを受信しました");
        RecvAuthPacket(packet, receiver, sender, m_isWhForwardedPacket);
        break;
    }
    case AODVTYPE_AUTHREP: {
        NS_LOG_DEBUG("AODVTYPE_AUTHREPを受信しました");
        RecvAuthReply(packet, receiver, sender, m_isWhForwardedPacket);
        break;
    }
    ...
    case AODVTYPE_DetecReq: {
        NS_LOG_DEBUG("AODVTYPE_DetecReqを受信しました");
...
//
bool
RoutingProtocol::IsPacketFromWh(Ptr<const Packet> p) const
{
    WhForwardTag tag;
    if (p->PeekPacketTag(tag) && tag.Get())
    {
        return true;
    }
    return false;
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src, bool fromWh)
{
    ...
    {
        NS_LOG_DEBUG("攻撃ノードがWH経由RREQを受信しました。ブロードキャストします。");
...
void
RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender, bool fromWh)
{
    ...
    {
        ProcessHello(rrepHeader, receiver, isRebroadcasted);
        return;
    }
    ...
    if (IsMyOwnAddress(rrepHeader.GetOrigin()))
    {
        ...
        //経路作成時間を取得
        uint32_t msgId = rrepHeader.GetMessageID();
        ...
        {
            NS_LOG_WARN("経路作成時間取得時にメッセージIDが一致しません" << msgId);
        }
...
RoutingProtocol::ProcessHello(RrepHeader& rrepHeader, Ipv4Address receiver, bool isRebroadcasted)
{
    ...
    if(rB > m_whNeighborThreshold && rA > m_whNeighborThreshold)
    {
        ...
        if(isRebroadcasted)
        {
            ...
            ofs << "WHノードの隣接ノード比率が閾値を超えているため、ステップ2検知を開始  ノードID：" << nodeId << "   シミュレーション時間：" << Simulator::Now() << std::endl;
            ofs  << " A=" << receiver
                 << " B=" << helloSender
                 << " rA=" << rA
                 << " rB=" << rB
        ...
        if (exclusiveNeighbors.size() < 2)
        {
            ...
            const double specialRatioThreshold = 2;
        ...
        if(isRebroadcasted || receiver == Ipv4Address("10.1.2.1") || receiver == Ipv4Address("10.1.2.2"))
        {
            std::ofstream ofs("test.log", std::ios::out | std::ios::app);
            uint32_t nodeId = GetObject<Node>()->GetId();
...
void
RoutingProtocol::StartStep3Detection(Ipv4Address startnode ,
                                     Ipv4Address target, 
                                     const std::set<Ipv4Address> NA, 
                                     const std::set<Ipv4Address> NB, 
                                     const std::set<Ipv4Address> commonNeighbors, 
                                     bool isRebroadcasted)
{
    ...
    if(isRebroadcasted)
    {
        std::ofstream ofs("test.log", std::ios::out | std::ios::app);
        uint32_t nodeId = GetObject<Node>()->GetId();
        ofs << "WHリンクがステップ3検知を開始  ノードID：" << nodeId << "   シミュレーション時間：" << Simulator::Now() << std::endl;
        ofs  << " A=" << startnode
             << " B=" << target
    ...
    entry.isRebroadcasted = isRebroadcasted;

    // 既存の Step3 結果が残っていれば削除
    auto itA = m_step3ResultTable.find(startnode);
    if (itA != m_step3ResultTable.end())
    {
        auto itB = itA->second.find(target);
        if (itB != itA->second.end())
        {
            NS_LOG_WARN("[Step3] A=" << startnode << " B=" << target
    ...
    // A → B の結果エントリをセット
    m_step3ResultTable[startnode][target] = entry;

    NS_LOG_INFO("[Step3] 結果テーブルを初期化: A=" << startnode
                    << " B=" << target
                    << " witness数=" << commonNeighbors.size());
    ...
    for(auto n : NA)
    {
        if (commonNeighbors.count(n) == 0) {
            ...
            if(n == target)
            {
                NS_LOG_DEBUG("判定開始ノード：" << startnode << " が判定対象ノード：" << n << "に送信停止依頼を行います。");
                
                //2 = 判定対象ノードに周辺ノードへ送信停止を依頼する
                SendVs(n, startnode, target, 2);
                continue;
            }
            NS_LOG_DEBUG("判定開始ノード：" << startnode << " が判定開始ノードの排他的隣接ノード：" << n << "に送信停止依頼を行います。");
    ...
    Ipv4Address A = startnode;
    Ipv4Address B = target;

    NS_LOG_DEBUG("[Step3] AUTHREP タイムアウトイベントを登録 A=" << A << " B=" << B);

    m_step3ResultTable[A][B].timeoutEvent =
    Simulator::Schedule(m_step3ReplyWaitTime, &RoutingProtocol::Step3TimeoutCheck, 
                        this, A, B, isRebroadcasted);
                                          ...
                                          bool isRebroadcasted)
{
    ...
    if (m_sendBlocked)
    {
        ...
        Simulator::Schedule(Seconds(0.05),
                            &RoutingProtocol::StartStep3DetectionRetry,
                            this, A, B, NA, NB, commonNeighbors, isRebroadcasted);
    ...
    StartStep3Detection(A, B, NA, NB, commonNeighbors,isRebroadcasted);
}
                              ...
                              NetDevice::PacketType type)
{
    ...
    //----------------------------------------------------------------------
    // Step3 (AUTH / AUTHREP) の監視処理
    //----------------------------------------------------------------------
    Step3_MonitorProcess(pStep3, protocol, myaddr);
...
RoutingProtocol::ProcessWhForwarding(Ptr<const Packet> originalPkt, uint16_t protocol, Ipv4Address myaddr)
{
    ...
    uint8_t l4 = ip.GetProtocol();
    bool shouldTunnel = false;

    //UDPの場合
    if(l4 == UdpL4Protocol::PROT_NUMBER)
    {
        UdpHeader udp;
        p->RemoveHeader(udp);
        //AODVの場合
        if (udp.GetDestinationPort() == AODV_PORT)
        {
            shouldTunnel = true;     //AODVに関するパケットを全て転送
        }
    }else if(ip.GetProtocol() == Icmpv4L4Protocol::PROT_NUMBER)
    {
        shouldTunnel = true;          // Ping も転送
    }
...
{
    NS_LOG_DEBUG("受信時のパケットタイプ：" << IdentifyAodvType(pkt));
    if(!m_isWhNode)
    {
   ...
   //SendTo(s, inner, toPeer.GetNextHop());
    s->SendTo(inner, 0, InetSocketAddress(m_whPeerIp, AODV_PORT));
...
std::string
RoutingProtocol::IdentifyAodvType(Ptr<const Packet> packet) const
{
    Ptr<Packet> p = packet->Copy();
    Ipv4Header ip;
    UdpHeader udp;
    ...
    switch(th.Get())
    {
        case AODVTYPE_RREQ:       return "RREQ";
        case AODVTYPE_RREP:       return "RREP";
        case AODVTYPE_RERR:       return "RERR";
        case AODVTYPE_RREP_ACK:   return "RREP_ACK";
        case AODVTYPE_VSR:        return "VSR";
        case AODVTYPE_AUTH:       return "AUTH";
        case AODVTYPE_AUTHREP:    return "AUTHREP";
        case AODVTYPE_STEP3RESULT:return "STEP3RESULT";
        default:                  return "AODV_UNKNOWN";
    }
}

void
RoutingProtocol::Step3_MonitorProcess(Ptr<Packet> p, uint16_t protocol, Ipv4Address myaddr)
{
    ...
    if (tHeader.Get() == AODVTYPE_AUTH)
    {
        ...
        if (ipSender == origin)
        {
            ...
            NS_LOG_DEBUG("[Step3][witness=" << myaddr << "] AUTH を origin="
                          << origin << " から直接受信");
        }
        else
        {
            ...
            NS_LOG_DEBUG("[Step3][witness=" << myaddr
    ...
    else if (tHeader.Get() == AODVTYPE_AUTHREP)
    {
        ...
        if (ipSender == target)
        {
            ...
            NS_LOG_DEBUG("[Step3][witness=" << myaddr << "] AUTHREP を target="
                          << target << " から直接受信");
        }
        else
        {
        ...
        //判定結果を送信（後ほど実装）
        SendStep3Result(origin, target, myaddr,tag);
...
void
RoutingProtocol::RecvVerificationStart(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src, bool fromWh)
{
    NS_LOG_FUNCTION(this);
    ...
    if(!m_routingTable.LookupRoute(vsh.GetOrigin(), toA))
    {
        ...
        ofs << Simulator::Now().GetSeconds() << "s Node " << nodeId 
            << " : ステップ3の送信停止・監視要求メッセージの判定開始ノード：" 
            << vsh.GetOrigin() << "への経路が存在しません。" << std::endl;
        ofs.close();
                                ...
                                bool fromWh)
{
    ...
    NS_LOG_DEBUG("認証メッセージ　A：" << A << "  B：" << B << "メッセージID：" << auth.GetID());
    ...
    if(!IsMyOwnAddress(B))
    {
        NS_LOG_DEBUG("認証パケットが自信宛ではないため、ドロップ");
        return;
    }
    ...
    if(!m_routingTable.LookupRoute(auth.GetOrigin(), toA))
    {
        ...
        ofs << "ノードID：" << nodeId << "   シミュレーション時間：" << Simulator::Now() << std::endl;
        ofs << " A=" << A << " B=" << B << "  メッセージID：" << auth.GetID() << std::endl;
        ofs.close();
    }

    NS_LOG_DEBUG("受信ノード：" << receiver << "が送信ノード："<< sender << "からの認証メッセージを受信しました。");
    ...
    if (IsMyOwnAddress(B))
    {
        SendAuthReply(A, B, auth.GetID());
    }
}

void
RoutingProtocol::RecvAuthReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender, bool fromWh)
{
    NS_LOG_FUNCTION(this);
    Ptr<Packet> copy = p->Copy();
    NS_LOG_DEBUG("受診者：" << receiver << "が送信者："<< sender << "からの認証返信メッセージを受信しました。");

    AuthReplyHeader authRep_copy;
    copy->RemoveHeader(authRep_copy);

    Ipv4Address A = authRep_copy.GetOrigin();   // 判定開始ノード
    Ipv4Address B = authRep_copy.GetTarget();     // 判定対象ノード

    NS_LOG_DEBUG("メッセージID：" << authRep_copy.GetID() << "[Step3] 判定開始ノード " << A
                  << " が判定対象ノード " << B
                  << " から AUTHREP を受信");
    ...
    if(!IsMyOwnAddress(A))
    {
        NS_LOG_DEBUG("自身宛ではないため、ドロップ");
        return;
    }

    if(isRebroadcasted)
    {
        NS_LOG_DEBUG("WHリンクが再ブロードキャストされた認証応答メッセージを受信しました。");

        std::ofstream ofs("test.log", std::ios::out | std::ios::app);
        uint32_t nodeId = GetObject<Node>()->GetId();

        ofs << "WHリンクが再ブロードキャスト認証応答メッセージを受信  ノードID：" << nodeId << "   シミュレーション時間：" << Simulator::Now() << std::endl;
        ofs << " A=" << A << " B=" << B  << std::endl;
        ofs.close();
    }
...
void
RoutingProtocol::Step3TimeoutCheck(Ipv4Address A, Ipv4Address B, bool isRebroadcasted)
{
...
void
RoutingProtocol::RecvStep3Result(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender, bool fromWh)
{
    NS_LOG_FUNCTION(this);
...
RoutingProtocol::RecvDetectionReq(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src, bool fromWh)
{
    ...
    if (p->RemovePacketTag(whReTag) && whReTag.Get())
    {
        NS_LOG_DEBUG("WHノードからの再ブロードキャスト、ステップ2の別経路要求メッセージを受信しました。送信者：" << src);

        if(m_isWhNode)
        {
            NS_LOG_DEBUG("入口側のノードが再ブロードキャスト、ステップ2の別経路要求メッセージを受信しました。処理を終了します。");
            return;
        }
    }
    ...
    if(m_isWhNode && fromWh)
    {
        NS_LOG_DEBUG("攻撃ノードがWH経由ステップ2の別経路要求メッセージを受信しました。ブロードキャストします。");
...
RoutingProtocol::EvaluateStep2AndAct(uint32_t detId, bool timedOut)
{
    ...
    // ★ erase前にコピーしておく（UAF防止）
    const Step2CollectEntry& sess = it->second;
    Ipv4Address originA = sess.originA;
    Ipv4Address targetB = sess.targetB;
    std::set<Ipv4Address> NB = sess.NB;
    bool isRebroadcasted = sess.isRebroadcasted;
    auto reports = sess.reports; // 必要なら（重いなら参照だけにする/使わないなら消す）
    ...
    for (const auto& rep : reports)
    {
        ...
        for (const auto& kv : hopMap)
        {
            ...
            ofs << "ステップ2の評価  ノードID：" << nodeId << "   シミュレーション時間：" << Simulator::Now() << std::endl;
            ofs << "ターゲットノード：" << targetB << "  レポーターノード：" << reporter << "  ピアノード：" << peer << "  ホップ数：" << int(hop) <<std::endl;
            ofs.close();

            NS_LOG_DEBUG("ステップ2の評価  検知ID：=" << detId
                         << " reporter=" << reporter
                         << " peer=" << peer
                         << " hop=" << int(hop));
    ...
    if (wormhole)
    {
        m_blacklist.insert(targetB);
        NS_LOG_WARN("[Step2] WH攻撃と判定しました. targetB blacklisted: " << targetB);
        ...
        if(isRebroadcasted)
        {
            ...
            ofs << "別経路により正常リンクを誤検知  ノードID：" << nodeId << "   シミュレーション時間：" << Simulator::Now() << std::endl;
            ofs << "ターゲットノード：" << targetB << std::endl;
            ofs.close();
    ...
    if(isRebroadcasted)
    {
        ...
        ofs << "WHリンクがステップ2を通過  ノードID：" << nodeId << "   シミュレーション時間：" << Simulator::Now() << std::endl;
        ofs << "ターゲットノード：" << targetB << std::endl;
        ofs.close();

    }

    // ★重要：有効測定がゼロなら Step3 に突入しない（まずは OOM 止める）
    // if (usedCount == 0)
    // {
    //     NS_LOG_WARN("[Step2] No valid hop measured (used=0). Hold/skip Step3. detId=" << detId);
    //     m_step2Collect.erase(it);
    //     return;
    // }
    ...
    for (auto itr = m_routingTable.m_ipv4AddressEntry.begin();
         itr != m_routingTable.m_ipv4AddressEntry.end(); ++itr)
    {
        const RoutingTableEntry& e = itr->second;
        if (e.GetHop() == 1 && e.GetFlag() == VALID &&
            e.GetNextHop() != Ipv4Address("127.0.0.1") &&
            e.GetNextHop() != Ipv4Address("10.255.255.255"))
        {
            neighborList.insert(e.GetDestination());
        }
    }
    ...
    // Step3開始（ここは必ず有限化：多重起動ガード/リトライ上限/タイマーCancelなど）
    StartStep3Detection(originA, targetB, neighborList, NB, commonNeighbors, isRebroadcasted);
...
void
RoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src, bool fromWh)
{
...
// void
// RoutingProtocol::ReceiveFromWhTunnel(Ptr<Socket> socket)
// {
//     Address from;
...
//         {
//             isWhForwarded = true;
//             NS_LOG_DEBUG("[WH出口] WHタグ付きパケットを受付 → 自ノード経由で再注入する");
//         }
//     }
//     else
//     {
//         NS_LOG_DEBUG("[WH出口] WHタグなしパケットを受信（通常の通信？）");
//     }

//     if (!pkt || pkt->GetSize() == 0)
//     {
//         NS_LOG_DEBUG("[WH出口] 受信パケットが NULL またはサイズ0のため破棄する");
//         return;
//     }

//     Ipv4Address myaddr = m_ipv4->GetAddress(1,0).GetLocal();

//     NS_LOG_DEBUG("[WH出口] " << myaddr
//                   << " が WHトンネルからパケット受信");

//     // // ① ここで Step3 の監視処理を実行
//     // Ptr<Packet> pStep3 = pkt->Copy();
//     // Step3_MonitorProcess(pStep3, Ipv4L3Protocol::PROT_NUMBER, myaddr);

//     // ② パケットが AODV または PING か判定
//     std::string typeStr = IdentifyAodvType(pkt);

//     NS_LOG_DEBUG("[WH出口] パケット種別 = " << typeStr);

//     // IdentifyAodvType の戻り値:
//     //  NOT_IP / NOT_UDP / NOT_AODV なら AODV ではない
//     //  それ以外 (RREQ/RREP/…/AODV_UNKNOWN) は AODV とみなす
//     if (typeStr != "NOT_IP" &&
//         typeStr != "NOT_UDP" &&
//         typeStr != "NOT_AODV")
//     {
//         NS_LOG_DEBUG("[WH出口] AODVパケットを再送信します");

//         // 無線インタフェース(例: index=1)の Ipv4InterfaceAddress を取得
//         Ipv4InterfaceAddress iface = m_ipv4->GetAddress(1, 0);

//         // そのインタフェースに紐づく AODV ソケットを取得
//         Ptr<Socket> aodvSocket = FindSocketWithInterfaceAddress(iface);
//         if (!aodvSocket)
//         {
//             NS_LOG_ERROR("[WH出口] AODV送信用ソケットが見つかりません (iface="
//                          << iface.GetLocal() << ")");
//             return;
//         }
        
//         bool oldBlock = m_sendBlocked;
//         m_sendBlocked = false; // 特例で送信許可

//         SendTo(aodvSocket, pkt->Copy(), Ipv4Address("255.255.255.255"));
...
//     // ----------------------------------------------------
//     // ② AODV でなければ、Ping(ICMPv4)かどうか確認
//     // ----------------------------------------------------
//     Ipv4Header ip;
//     Ptr<Packet> pp = pkt->Copy();

//     if (!pp->RemoveHeader(ip))
//     {
//         NS_LOG_DEBUG("[WH出口] IPv4ヘッダが読めないためパケットを破棄します");
//         return;
//     }

//     if (ip.GetProtocol() == Icmpv4L4Protocol::PROT_NUMBER)
//     {
//         NS_LOG_DEBUG("[WH出口] ICMP (Ping) を再送信します");

//         Ipv4Address dst = ip.GetDestination();

//         // 宛先への経路を AODV のルーティングテーブルから取得
//         RoutingTableEntry rt;
//         if (!m_routingTable.LookupRoute(dst, rt))
//         {
//             NS_LOG_WARN("[WH出口] Ping 宛先 " << dst
//                         << " への経路が見つからないため破棄");
//             return;
//         }
...
//         if (!route)
//         {
//             NS_LOG_ERROR("[WH出口] ルート情報が不正です (route==nullptr)");
//             return;
//         }

//         // Ipv4L3Protocol を取得
//         Ptr<Ipv4L3Protocol> ipv4 = m_ipv4->GetObject<Ipv4L3Protocol>();
//         if (!ipv4)
//         {
//             NS_LOG_ERROR("[WH出口] Ipv4L3Protocol が取得できません");
//             return;
//         }

//         // 送信元アドレスは route 内の Source を利用
//         Ipv4Address src = route->GetSource();

//         bool oldBlock = m_sendBlocked;
//         m_sendBlocked = false; // WH 転送は送信停止フラグに関係なく送信
...
//         NS_LOG_DEBUG("[WH出口] AODV でも Ping でもないためパケットを破棄します");

//         return;
//     }
//     NS_LOG_DEBUG("[WH出口] 不明なパケットのため破棄する");
// }
